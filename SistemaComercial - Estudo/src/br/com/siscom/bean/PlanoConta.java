 ...
 */
@Entity
@Table(name = "plano_conta",schema = "")
@NamedQueries({@NamedQuery(name = "PlanoConta.findByCodPlanoConta", query = "SELECT p FROM PlanoConta p WHERE p.codPlanoConta = :codPlanoConta"), @NamedQuery(name = "PlanoConta.findByDescricaoPlanoConta", query = "SELECT p FROM PlanoConta p WHERE p.descricaoPlanoConta = :descricaoPlanoConta")})
public class PlanoConta implements Serializable {
    @Transient
    private PropertyChangeSupport changeSupport = new PropertyChangeSupport(this);
    private static final long serialVersionUID = 1L;
    @Id @GeneratedValue(strategy=GenerationType.IDENTITY)
    @Column(name = "COD_PLANO_CONTA", nullable = false)
