...
import br.com.siscom.bean.PedidoDet;
...
public class PedidoCompra extends JPanel {
    ...
    private void initComponents() {
        ...
        query = entityManager.createQuery("SELECT P FROM PedidoCab P");
    ...
    private void deleteDetailButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteDetailButtonActionPerformed
        ...
        PedidoCab P = list.get(masterTable.convertRowIndexToModel(index));
        Collection<PedidoDet> ps = P.getPedidoDetCollection();
        int[] selected = detailTable.getSelectedRows();
        List<PedidoDet> toRemove = new ArrayList<PedidoDet>(selected.length);
        for (int idx=0; idx<selected.length; idx++) {
            selected[idx] = detailTable.convertRowIndexToModel(selected[idx]);
            int count = 0;
            Iterator<PedidoDet> iter = ps.iterator();
            while (count++ < selected[idx]) iter.next();
            PedidoDet p = iter.next();
            toRemove.add(p);
    ...
    private void newDetailButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_newDetailButtonActionPerformed
        ...
        Collection<PedidoDet> ps = P.getPedidoDetCollection();
        if (ps == null) {
            ps = new LinkedList<PedidoDet>();
            P.setPedidoDetCollection(ps);
        }
        PedidoDet p = new PedidoDet();
    ...
    private void deleteButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteButtonActionPerformed
        ...
        List<PedidoCab> toRemove = new ArrayList<PedidoCab>(selected.length);
        for (int idx=0; idx<selected.length; idx++) {
            PedidoCab P = list.get(masterTable.convertRowIndexToModel(selected[idx]));
            toRemove.add(P);
            entityManager.remove(P);
        }
